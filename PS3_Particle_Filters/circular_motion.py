...
class robot:
    ...
    def move(self, motion, tolerance = 0.001): # Do not change the name of this function
        ...
        # apply noise
        steering2 = random.gauss(alpha, self.steering_noise)
        distance2 = random.gauss(d, self.distance_noise)

        # excute motion
        tan_beta = d * tan(alpha) / self.length
        beta = atan(tan_beta)
        theta = self.orientation

        if abs(tan_beta) < tolerance:
            result.x = self.x + d * cos(theta)
            result.y = self.y + d * sin(theta)
            result.orientation = (theta + beta) % (2.0 * pi)
        else:
            radius = d / tan_beta
            Cx = self.x - radius * sin(theta)
            Cy = self.y + radius * cos(theta)
            result.x = Cx + radius * sin(theta + tan_beta)
            result.y = Cy - radius * cos(theta + tan_beta)
            result.orientation = (theta + beta) % (2.0 * pi)     
